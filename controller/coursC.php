    ...
    public function addCours($id_cours, $titre, $proprietaire, $prix, $description, $image, $category,$pdf)
{
    ...
    public function listCoursAscendingByProp()
    {
        $db = config::getConnexion();
        $sql = "SELECT * FROM cours ORDER BY proprietaire ASC"; // Modifier la colonne selon votre structure de base de données
        $query = $db->prepare($sql);
        $query->execute();
        $cours = $query->fetchAll();
        return $cours;
    ...
    // Fonction pour récupérer la liste des cours triés par catégorie ascendant
    public function listCoursDescendingByProp()
    {
        $db = config::getConnexion();
        $sql = "SELECT * FROM cours ORDER BY proprietaire DESC"; // Modifier la colonne selon votre structure de base de données
        $query = $db->prepare($sql);
        $query->execute();
        $cours = $query->fetchAll();
        return $cours;
